...
class RectItem(pg.GraphicsObject):
    ...
    def updateData(self, *args, **kargs):
        ...
        try:
            self.opts['text_list']=kargs['text_list']
        except:
            None
    ...
    def _generate_picture(self):
        ...
        for item in self.opts['rect']:
            ...
            #     painter.drawText(item,self.opts['text_list'][self.opts['rect'].index(item)])
            #     painter.restore()
            # except:
            #     None
...
class SimulationTimingPlotWidget(pg.PlotWidget):
    raw_item = ''
    input_key_event = ''
    qapp = ''
    summary_box = ''
    ...
    def set_summary_box(self,summary_box):
        self.summary_box = summary_box

    def set_event_tracer_item(self,logic_evt_tracker):
        self.die_event_tracker = logic_evt_tracker['die']
        self.plane_event_tracker = logic_evt_tracker['plane']
        self.sram_event_tracker = logic_evt_tracker['sram']
        self.dram_event_tracker = logic_evt_tracker['dram']
        self.hit_ratio_tracker = logic_evt_tracker['hit_ratio']
    ...
    def show_plot_item(self,*args):
        ...
        y_range_dict = dict()
        x_range_dict = list()
        summary_string = str(self.hit_ratio_tracker)+'\n'
        ...
        y_range_dict = dict()
        x_range_dict = list()
        for item in self.sram_event_tracker:
            ...
            if (item['start_time'] >= time_x_range['start']) & (item['start_time'] < time_x_range['end']):
                ...
                # try:
                #     if item['desc'] != '':
                #         text_item = pg.TextItem(item['desc'], (255, 160, 122))
                #         text_item.setPos(item['start_time'], sram_y_height+(cmd % 4)*0.25-0.75)
                #         text_item.setFont(QtGui.QFont('Times', 10))
                #         # text_item.setTextWidth(4)
                #         self.plotItem.addItem(text_item)
                # except:
                #     None
                item = QtCore.QRectF(x_start_time, sram_y_height-1+(cmd % 4)*0.25, latency, 0.25)
                if cmd not in y_range_dict:
                    y_range_dict[cmd] = list()
                    y_range_dict[cmd].append(item)
                else:
        ...
        x_range_dict = list()
        text_list = list()
        for item in self.dram_event_tracker:
            ...
            if (item['start_time'] >= time_x_range['start']) & (item['start_time'] < time_x_range['end']):
                ...
                #     if item['desc'] != '':
                #         text_item = pg.TextItem(item['desc'], (255, 160, 122))
                #         text_item.setPos(item['start_time'], dram_y_height+(cmd % 4)*0.25-0.75)
                #         text_item.setFont(QtGui.QFont('Times', 10))
                #         # text_item.setTextWidth(4)
                #         self.plotItem.addItem(text_item)
                #         text_list.append(item['desc'])
                # except:
                #     None

                item = QtCore.QRectF(x_start_time, dram_y_height-1+(cmd % 4)*0.25, latency, 0.25)
        ...
        for cmd_idx in y_range_dict.keys():
            ...
            item = RectItem(x=x_range_dict,rect=y_latency_group,text_list = text_list,symbol=cmd_symbol,name=cmd_name,bursh=(cmd_color))

            self.plotItem.addItem(item)

        self.summary_box.setPlainText(summary_string)
